...
async function getHrAnalytics(req, res) {
    try {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const count = await Attendance.countDocuments({ date: { $gte: d, $lt: nextD } });
            last7Days.push({
                date: d.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric' }),
                present: count,
                absent: Math.max(0, totalEmployees - count)
            });
        }
        ...
        res.status(200).json({
            ...
            data: {
                ...
                leaveDistribution: leaveTypes.map(l => ({ name: l._id, value: l.count }))
            }
        });
    } catch (error) {
...
async function getRankings(req, res) {
    try {
        const employees = await User.find({}).select('name email role');
        const rankings = await Promise.all(employees.map(async (emp) => {
            const totalPresent = await Attendance.countDocuments({ employee: emp._id, status: 'Present' });
            const totalLate = await Attendance.countDocuments({ employee: emp._id, isLate: true });
            const approvedLeaves = await Leave.countDocuments({ employee: emp._id, status: 'Approved' });
            const score = Math.max(0, (totalPresent * 2) - (totalLate * 0.5) - (approvedLeaves * 0.3));
            return { id: emp._id, name: emp.name, role: emp.role, score: parseFloat(score.toFixed(1)), totalPresent, totalLate, approvedLeaves };
        }));
        rankings.sort((a, b) => b.score - a.score);
        const rankedList = rankings.map((r, idx) => ({ ...r, rank: idx + 1 }));
        res.status(200).json({ success: true, data: rankedList });
    } catch (error) {
